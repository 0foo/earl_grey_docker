...
* `bin/batch-dashboard` — one-shot view of everything: job counts by status on the queue, a per-array child breakdown for anything RUNNING/FAILED, and a recent CloudWatch log tail, all in a single command.
